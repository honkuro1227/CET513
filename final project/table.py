...
#        dff=df[['veh_type','event_type','Dwell_time']]
        dff=df[['Zone','Mean_SPD']]
...
#        pauses[pauses["pause_end"] > pauses["pause_start"]]
        dff.groupby(by=['Zone']).mean().reset_index().to_csv('zone_speed.csv',index=False)
#        dff.groupby(by=[,'event_type','veh_type']).mean().reset_index().to_csv('zone_dwell.csv',index=False)
